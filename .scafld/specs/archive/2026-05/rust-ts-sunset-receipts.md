...
Status: completed
Current phase: review rerun
Next: done
Reason: task completed
Blockers: none.
Allowed follow-up command: `none`
Latest runner update: 2026-05-20T02:18:02Z
Review gate: pass
...
The deletion slice is now implemented in the active workspace. The store,
journal, history, parent/child tree proof acceptance, runtime verifier policy,
importer migration, cloud sibling audit, and importer audit guardrail slices
have landed. The remaining work for this spec is review rerun and completion,
...
- `packages/core/src/receipts/**` is deleted.
- `packages/core/package.json` no longer exposes the `./receipts` subpath.
- `packages/contracts/src/schemas/local-receipt.ts` is deleted, and
  `packages/contracts/src/index.ts` no longer exports retired local receipt
  contracts.
- Active OSS imports no longer reference `@runxhq/core/receipts` or
  `../receipts/index.js`.
- Sibling cloud checkout audit no longer finds active retired local receipt
  imports or retired local receipt field usage.
...
- Latest audit scanned 1378 files including the sibling `../cloud` checkout
  and reported: `active_blocker=0`, `fixture_archive=11`,
  `generated_stale_artifact=16`, `migrated=423`, `false_positive=71`.
- Active blocker kinds: none.
- Local generated runtime receipt data under `.runx/`, including
  `packages/cli/.runx/receipts/**`, is excluded from the importer audit so the
  gate does not count developer-local receipt output as live source drift.
- The prior OSS live retired-import blocker list is cleared.
- Prefix-based legacy graph/receipt inference in
  `packages/runtime-local/src/runner-local/history.ts` has been removed from
  live read paths. Paused-run logical schemas now use harness schema ids.
...
- Runtime pseudo-signature policy: implemented. Runtime receipt proof now uses
  `RuntimeReceiptSignaturePolicy` with explicit `local_development`,
  `production`, and `production_without_verifier` modes. Local pseudo
  signatures are allowed only by the explicit local development policy;
  production rejects `sig:` pseudo signatures before delegating, and production
  without a verifier fails closed.
- Remaining audit drift: `scripts/check-receipt-importers.ts` no longer flags
  explicit local-development/test pseudo-signature contexts as
  `active_blocker`; the live importer audit now reports zero active blockers.
- TS receipt imports: cleared. `packages/core/src/receipts/**` is deleted,
  `@runxhq/core/receipts` is no longer exported, `local-receipt.ts` is deleted
  from contracts, and active TS readers/writers use harness receipts.
...
- Remaining caveat: production Ed25519 signer materialization is still a
  follow-up, but production acceptance now fails closed without an injected
...
Status: completed 2026-05-20 for live importer deletion guardrails.
...
Status: completed 2026-05-20 for the live OSS implementation and sibling cloud
audit; archive/generated stale artifacts remain classified as non-blocking.
...
- What production signer supplies real Ed25519 signatures to
  `RuntimeReceiptSignaturePolicy::production`: local key material, hosted
  verifier/signer injection, or a runtime signer trait supplied by the caller?
  The acceptance policy is implemented; signer materialization remains a
  follow-up.
...
Summary: Human-reviewed override accepted: Claude verify review found no substantive blockers; workspace-mutation guard was caused by spec text correction during review, and the two low cleanup findings were addressed with pnpm docs:api plus boundary-check cleanup.

Attack log:
- `review gate`: manual human audit -> clean (Claude verify review found no substantive blockers; workspace-mutation guard was caused by spec text correction during review, and the two low cleanup findings were addressed with pnpm docs:api plus boundary-check cleanup.)
